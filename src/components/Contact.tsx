import ButterflyDecor from './ButterflyDecor';

export default function Contact() {
    return (
        <section id="contact" className="section-container bg-pure-white">
            <div className="relative">
                <ButterflyDecor
                    className="absolute -top-8 -left-4 text-grey-200 opacity-50"
                    size="md"
                    animated={false}
                />
                <h2 className="section-title">Get In Touch</h2>
            </div>

            <div className="max-w-3xl mx-auto">
                <p className="text-lg text-grey-600 text-center mb-12">
                    I'm always open to discussing new projects, creative ideas, or opportunities to be part of your vision.
                </p>

                <div className="grid md:grid-cols-2 gap-6 mb-12">
                    {/* Email */}
                    <a
                        href="mailto:your.email@example.com"
                        className="card group text-center"
                    >
                        <div className="w-12 h-12 bg-grey-900 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:bg-grey-700 transition-colors">
                            <svg className="w-6 h-6 text-pure-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" />
                            </svg>
                        </div>
                        <h3 className="font-display font-semibold text-lg mb-2">Email</h3>
                        <p className="text-grey-600">your.email@example.com</p>
                    </a>

                    {/* LinkedIn */}
                    <a
                        href="https://linkedin.com/in/yourprofile"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="card group text-center"
                    >
                        <div className="w-12 h-12 bg-grey-900 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:bg-grey-700 transition-colors">
                            <svg className="w-6 h-6 text-pure-white" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M19 0h-14c-2.761 0-5 2.239-5 5v14c0 2.761 2.239 5 5 5h14c2.762 0 5-2.239 5-5v-14c0-2.761-2.238-5-5-5zm-11 19h-3v-11h3v11zm-1.5-12.268c-.966 0-1.75-.79-1.75-1.764s.784-1.764 1.75-1.764 1.75.79 1.75 1.764-.783 1.764-1.75 1.764zm13.5 12.268h-3v-5.604c0-3.368-4-3.113-4 0v5.604h-3v-11h3v1.765c1.396-2.586 7-2.777 7 2.476v6.759z" />
                            </svg>
                        </div>
                        <h3 className="font-display font-semibold text-lg mb-2">LinkedIn</h3>
                        <p className="text-grey-600">linkedin.com/in/yourprofile</p>
                    </a>

                    {/* GitHub */}
                    <a
                        href="https://github.com/yourusername"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="card group text-center"
                    >
                        <div className="w-12 h-12 bg-grey-900 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:bg-grey-700 transition-colors">
                            <svg className="w-6 h-6 text-pure-white" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z" />
                            </svg>
                        </div>
                        <h3 className="font-display font-semibold text-lg mb-2">GitHub</h3>
                        <p className="text-grey-600">github.com/yourusername</p>
                    </a>

                    {/* Twitter/X */}
                    <a
                        href="https://twitter.com/yourusername"
                        target="_blank"
                        rel="noopener noreferrer"
                        className="card group text-center"
                    >
                        <div className="w-12 h-12 bg-grey-900 rounded-full flex items-center justify-center mx-auto mb-4 group-hover:bg-grey-700 transition-colors">
                            <svg className="w-6 h-6 text-pure-white" fill="currentColor" viewBox="0 0 24 24">
                                <path d="M18.244 2.25h3.308l-7.227 8.26 8.502 11.24H16.17l-5.214-6.817L4.99 21.75H1.68l7.73-8.835L1.254 2.25H8.08l4.713 6.231zm-1.161 17.52h1.833L7.084 4.126H5.117z" />
                            </svg>
                        </div>
                        <h3 className="font-display font-semibold text-lg mb-2">Twitter</h3>
                        <p className="text-grey-600">@yourusername</p>
                    </a>
                </div>

                {/* Footer */}
                <div className="text-center pt-8 border-t border-grey-200">
                    <div className="mb-4">
                        <ButterflyDecor size="sm" className="inline-block text-grey-400" animated={false} />
                    </div>
                    <p className="text-grey-500 text-sm">
                        © {new Date().getFullYear()} Your Name. All rights reserved.
                    </p>
                </div>
            </div>
        </section>
    );
}
